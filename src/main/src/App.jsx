...
class App extends Component {
  ...
  render() {
    ...
    const views = [
      { view: <ScheduleEditorContainer route={this.state.value} /> },
      { view: <ExcelImportContainer route={this.state.value} /> },
      { view: <PaintAppContainer route={this.state.value} /> },
      { view: <DriverPerformanceContainer route={this.state.value} /> }
    ];
    return (
      <Grid>
        <MagnaHeader
        ...
        <Grid style={{ height: '80%' }}>
          <div style={{ paddingTop: '55px' }}>
            <SwipeableViews
              ...
              {views.map(view => (
                <TabContainer dir={theme.direction}>{view.view}</TabContainer>
              ))}
            </SwipeableViews>
          </div>
        </Grid>
