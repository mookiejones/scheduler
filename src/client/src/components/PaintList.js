...
let sortFn = function(a, b){
    let roundA = parseInt(a[2]);
    let posA = parseInt(a[3]);
    let roundB = parseInt(b[2]);
    let posB = parseInt(b[3]);
    let qtyA = parseInt(a[9]);
    let qtyB = parseInt(b[9]);
...
export default class PaintList extends Component {
    ...
    checkOut(data, rowIdx) {
      ...
      let query = "id=" + parseInt(data[0]) + "&pickedBy=" + this.state.currentUser.id;
